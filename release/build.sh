...
# Build paseto-cli binaries for Linux, macOS and Windows, packaging them in their
...
build() {
  ...
  if [ -n "$_version" ]; then
    _build_args+=(-ldflags "-X go.hackfix.me/paseto-cli/app/context.vcsVersion=${_version}")
  fi
